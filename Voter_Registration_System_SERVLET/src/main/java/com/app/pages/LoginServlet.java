	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			if (user == null) {
				...
				/*
				 * // 5. create a cookie holding valid user details Cookie c1 = new
				 * Cookie("user_info", user.toString()); // 6. Add the cookie in resp header
				 * response.addCookie(c1); pw.print("successful login!!!!");// contents are
				 * discarded ! // pw.flush();
				 */	
